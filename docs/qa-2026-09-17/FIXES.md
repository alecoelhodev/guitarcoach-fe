...
| **QA-06** zero-task routine → "No active session"         | Start Practice was enabled on an empty routine, at **three** entry points                                                                                                                     | Disabled with an explanation in the builder, on the routine card, and on Home's "Today's practice" card — the third was found by the browser re-test, not by desk work. Deliberately does **not** touch the session screen's empty branch — blank practice is spec 07 |
...
After the fixes were committed, every web-reachable defect was re-walked against the running
app (Expo web dev server on `:8081`, the local Docker backend, real accounts) by driving Chrome
over the DevTools Protocol. **This found one thing the desk work missed** — see QA-06 below.

| Check       | Result                                                                                                                                                                                                                                                                                                                                                                             |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| QA-01       | Followed the report's own steps: signed in as A, started practice, typed `PRIVATE QA ACCOUNT A NOTES`, expired the cookie, let Finish Session 401. The persisted session carried a `userId`; after the 401 it read `{"state":{"tasks":[]},"version":1}`; account B's Home offered **no** resume prompt, no notes, no routine.                                                      |
| QA-03       | A real 16,044-byte mono 8 kHz WAV — the report's own file shape — uploaded through the real picker and appeared as `take-1.wav · WAV`. **Negative control:** disabling just the `file.file` branch reproduced "That file can't be uploaded" on the same file, then restoring it fixed it again.                                                                                    |
| QA-04       | Account B's empty Active tab renders both Create Routine and Ask AI Coach.                                                                                                                                                                                                                                                                                                         |
| QA-05       | Starting practice raised no resume dialog over the new session; navigating back to Home mid-session **did** offer Resume, which is the behaviour that matters and the reason this is a route guard rather than a hydration latch. Cold-loading `/session/active` and finishing landed on `/` with the session saved and the store cleared — not stranded on an empty live session. |
| QA-07       | The session at `2026-09-18T02:57Z` is 22:57 on the 17th in the host zone: History grouped it under `2026-09-17`, agreeing with the detail screen's `9/17/2026`. With the viewer overridden to `Asia/Tokyo` the same session correctly moved to `2026-09-18`.                                                                                                                       |
| QA-08       | Injecting HTTP 500 on `/recordings/{id}/download-url` produced "Something went wrong on our end", **not** the expired-link copy, with Get a new link preserved; restoring the endpoint recovered playback.                                                                                                                                                                         |
| QA-09       | An Instant Create prompt and the selected mode both survived a 390×844 → 1440×900 resize.                                                                                                                                                                                                                                                                                          |
| QA-10       | A 500 from the planner surfaced "Something went wrong on our end / Try again in a moment." with a Try again button. Before, this screen rendered nothing at all.                                                                                                                                                                                                                   |
| QA-02 (web) | The `Card` change is cross-platform, so the web paths that already worked were re-checked: Library task card, History session card and the Home avatar all still navigate.                                                                                                                                                                                                         |

One nuance worth recording: a fulfilled fault response without CORS headers is blocked by the
browser, so `fetch` rejects and the app honestly reports "No connection" rather than the
injected status. That is correct behaviour — the first QA-08 and QA-10 runs were measuring the
injection, not the app.

### QA-06 was incomplete, and the browser pass is what caught it

The committed fix disabled Start Practice in the routine builder and on the routine card. It
missed a **third entry point**: Home's "Today's practice" card has its own Start Practice
button, and `use-todays-practice` derives its pick as simply the newest active routine — which
can perfectly well have no tasks. A routine created by hand or by Instant Create with zero
tasks was badged "0 tasks" on Home and still offered Start Practice, which opened "No active
session" exactly as reported.

`todays-practice-card.tsx` now disables the button and replaces the empty task line with "Add a
task to this routine before practising it.", with two regression tests. Re-tested: Home shows
the hint and the button no longer navigates.
...
- **QA-02 is the one defect that could not be re-tested, and it is the native one.** Expo Go on
  a simulator has no automation driver, so the native tap behaviour rests on the SDK 57 docs,
  the `pressLinkTarget` tests, and a clean iOS export. Its web side was re-checked. **This is
  the check most worth doing by hand before sign-off.**
- The browser pass ran headless at 390×844 and 1440×900 only. No real device, and no
  Mobile Safari.
